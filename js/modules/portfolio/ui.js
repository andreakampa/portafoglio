...
export const CartPanel = {
    ...
    render() {
        ...
        Cart.items.forEach(item => {
            ...
            if (item.type === 'buy') {
                const costNative = item.qty * item.price + (item.commissionNative ?? (item.valuta === 'USD' ? item.commission * rate : item.commission));
                const costEur = toEur(costNative);
                totalBuyEur += costEur;
                html += `
                    <div class="cart-item cart-item-buy">
                        ...
                        <div class="cart-item-detail">
                            ${Calc.fmt(item.qty, 4)} az. × ${s} ${Calc.fmt(item.price)} + comm. € ${Calc.fmt(item.commission)}
                            ${item.valuta === 'USD' ? `<span class="cart-eur-hint">(≈ $ ${Calc.fmt(item.commissionNative ?? item.commission * rate)})</span>` : ''}
                        </div>
                        <div class="cart-item-total buy-color">
                            Costo: <b>${s} ${Calc.fmt(costNative)}</b>
                            ${item.valuta === 'USD' ? `<span class="cart-eur-hint">≈ € ${Calc.fmt(costEur)}</span>` : ''}
                        </div>
                        <div class="cart-item-pmc">Nuovo PMC: <b>${s} ${Calc.fmt(item.newPmc)}</b> &nbsp;|&nbsp; Q.tà tot: <b>${Calc.fmt(item.newQty, 4)}</b></div>
                    </div>`;
            } else {
