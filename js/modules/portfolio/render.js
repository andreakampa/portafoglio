    ...
    valuta: v = (p.valuta || 'EUR').toUpperCase()
            ...
            tr.innerHTML = `
                ...
                <td class="${pnl >= 0 ? 'text-cyan fw-bold' : 'neg-loss'}">
                    ${att > 0
    ? `${pnlEur < 0 ? '-' : ''}${s} ${Calc.fmt(Math.abs(currency === 'EUR' ? pnlEur : cv(pnl)))}<br><span id="${rowId}" class="fs-xs">(${Calc.fmtSign(pnlP)}%)</span>${pos?.fxEffect != null ? `<br><span style="font-size:9px;color:var(--text-muted);font-weight:400;">⇄ fx: ${pos.fxEffect >= 0 ? '+' : ''}€ ${Calc.fmt(pos.fxEffect)}</span>` : ''}`
    : '—'}
                </td>
                <td>
                    ${att > 0
